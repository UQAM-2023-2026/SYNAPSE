...
public:
    ...
    // Handle incoming /full message (token exchange)
    // fromMale: true if received on MALE port, false if on FEMALE
    void handleFullMessage(MicroOscMessage& msg, bool fromMale);
    
    // Handle incoming /sync message (broadcast trigger)
    void handleSyncMessage(MicroOscMessage& msg, bool fromMale);
...
private:
    ...
    bool _syncForwarded;        // Have we forwarded /sync? (prevents infinite loop)
    ...
    // Send /sync broadcast (triggers all rhizomes)
