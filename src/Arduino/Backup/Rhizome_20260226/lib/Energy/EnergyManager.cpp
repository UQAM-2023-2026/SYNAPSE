...
void EnergyManager::updateEnergyForState(RhizomeState state, bool bothDisconnected) {
    ...
    switch (state) {
        case RhizomeState::DEAD:
            if (bothDisconnected) {
                // DEAD + disconnected: slow regen to 50%
    ...
    // Edge detection: DEAD and energy just became > 0% (restored)
    // Exit DEAD immediately when energy starts regenerating
    if (state == RhizomeState::DEAD && wasDeadAndLow && energy > MIN_ENERGY) {
        Serial.println("[ENERGY] Restored - exiting DEAD");
