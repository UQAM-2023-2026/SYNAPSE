...
void onConnected1() {
  Serial.println("---");
  Serial.println("[CONN] Rhizome 1 connected!");
...
void onDisconnected1() {
  Serial.println("---");
  Serial.println("[CONN] Rhizome 1 disconnected");
  Serial.println("---");
...
void onConnected2() {
  Serial.println("---");
  Serial.println("[CONN] Rhizome 2 connected!");
...
void onDisconnected2() {
  Serial.println("---");
  Serial.println("[CONN] Rhizome 2 disconnected");
  Serial.println("---");
...
void updateConnectionState2() {
  ...
  if (isConnected2 && (now - lastEnergy2_Time > CONNECTION_TIMEOUT * 3)) {
  ...
  switch (conn2_State) {
    ...
    case CONN_CONNECTED:
      if (!pinState) {
        conn2_State = CONN_DEBOUNCING_DISCONNECT;
        debounce2_Start = now;
      }
      break;
    ...
    case CONN_DEBOUNCING_DISCONNECT:
      if (pinState) {
        conn2_State = CONN_CONNECTED;
      } else if (now - debounce2_Start >= DISCONNECT_DEBOUNCE) {
        conn2_State = CONN_DISCONNECTED;
        isConnected2 = false;
        onDisconnected2();
      }
...
void handleNode2Message(MicroOscMessage &msg) {
  ...
  if (msg.checkOscAddress("/discover_list")) {
    ...
    // FALLBACK: If we receive message and not connected, force connection
    if (!isConnected2) {
      Serial.println("[FALLBACK] Pogopin 2 - Forcing connection (FLAG not working, but receiving OSC)");
      isConnected2 = true;
      conn2_State = CONN_CONNECTED;
      onConnected2();
    }
...
void loopSendToTouch() {
  // Determine which IDs and energy values to send
  // If both connected, prioritize rhizome 1
  int idToSend = 0;
  int energyToSend = 0;
  
  if (isConnected1) {
    idToSend = rhizome1_ID;
    energyToSend = rhizome1_Energy;
  } else if (isConnected2) {
    idToSend = rhizome2_ID;
    energyToSend = rhizome2_Energy;
  }
...
void SerialLoop() {
  // Debug: Periodically check FLAG pin states
  static unsigned long lastDebug = 0;
  unsigned long now = millis();
  
  if (now - lastDebug >= 2000) {  // Every 2 seconds
    Serial.print("[DEBUG] FLAG1: ");
    Serial.print(digitalRead(POGOPIN1_FLAG_PIN) ? "HIGH" : "LOW");
    Serial.print(" | FLAG2: ");
    Serial.print(digitalRead(POGOPIN2_FLAG_PIN) ? "HIGH" : "LOW");
    Serial.print(" | Conn1: ");
    Serial.print(isConnected1 ? "YES" : "NO");
    Serial.print(" | Conn2: ");
    Serial.print(isConnected2 ? "YES" : "NO");
    
    // Show time since last energy message
    if (isConnected1) {
      Serial.print(" | R1_LastMsg: ");
      Serial.print((now - lastEnergy1_Time) / 1000);
      Serial.print("s ago");
    }
    if (isConnected2) {
      Serial.print(" | R2_LastMsg: ");
      Serial.print((now - lastEnergy2_Time) / 1000);
      Serial.print("s ago");
    }
